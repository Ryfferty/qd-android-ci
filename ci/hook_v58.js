...
Java.perform(function(){
  ...
  for(var an in adds){
    n++;
    try{ dumpR('uk(PAY|'+an+')', UK.call(FK,PAY,PAY.length,adds[an],adds[an].length)); }
    catch(e){S({m:'uk|'+an+' 异常: '+String(e).slice(0,200)});}
    try{ dumpR('uksf(PAY|'+an+'|空)', UKSF.call(FK,PAY,PAY.length,adds[an],adds[an].length,EMPTY)); }catch(e){S({m:'uksf1|'+an+' 异常: '+String(e).slice(0,140)});}
    try{ dumpR('uksf(PAY|空|'+an+')', UKSF.call(FK,PAY,PAY.length,EMPTY,0,adds[an])); }catch(e){S({m:'uksf2|'+an+' 异常: '+String(e).slice(0,140)});}
  }
  S({m:'UK类型: '+(typeof UK)+' UKSF: '+(typeof UKSF)});
  // 头/尾特殊: 整 qd (含 8B 头) 当 data
  var QD=Java.array('byte',hex2arr((P.qd_full_hex||'').slice(0, 2*13000)));
  if(P.qd_full_hex){
    try{ dumpR('uk(QD|空)', UK.call(FK,QD,QD.length,EMPTY,0)); }catch(e){S({m:'ukQD err '+String(e).slice(0,60)});}
  }
  try{ dumpR('uk(PAY|dk32)', UK.call(FK,PAY,PAY.length,dkA,32)); }catch(e){}
  try{ dumpR('uk(PAY|MD5)', UK.call(FK,PAY,PAY.length,MD5,16)); }catch(e){}
  // trailer u32=4 → 池子选块假说: uk(data,len,sel16,16) 五块全试
  ...
  for(var bi=0;bi<5;bi++){
    ...
    try{ dumpR('uk(PAY|pool'+bi+')', UK.call(FK,PAY,PAY.length,blk,16)); }catch(e){S({m:'pool'+bi+' err '+String(e).slice(0,80)});}
    try{ dumpR('ukd(PAYb64|pool'+bi+')', UK.call(FK,B64.decode(b64str,0),PAY.length,blk,16)); }catch(e){}
  }
  S({m:'=== v18 done ==='});
});