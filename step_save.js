  ...
  for(var i=0;i<fs.length;i++){fs[i].setAccessible(true);var v=fs[i].get(r);
   ...
   if(v.getClass().getName()==='[B'){var s2=null;try{s2=Java.use('java.lang.String').$new(v,'UTF-8')+'';}catch(e){}
    ...
    if(v.length>100){var cn=0;for(var k=0;k<s2.length;k++){var c=s2.charCodeAt(k);if(c>=0x4e00&&c<=0x9fff)cn++;}
     ...
     try{var f2=Java.use('java.io.FileOutputStream').$new(DIR+'plain_save.txt',false);
      f2.write(Java.use('java.lang.String').$new(s2).getBytes('UTF-8'));f2.close();S({m:'★ 明文已落盘 /data/local/tmp/plain_save.txt'});}catch(e){}}
...
Java.perform(function(){
 ...
 S({m:'=== 单步 save ==='});
 try{FU.INSTANCE.value.save(ctx,keyA,ver);S({m:'save OK'});}catch(e){S({m:'save 异常 '+String(e).slice(0,160)});}
 try{S({m:'复查 isHasKey='+FU.INSTANCE.value.isHasKey()+' getKey='+clip(FU.INSTANCE.value.getKey(),40)});}catch(e){}
});
