    ...
    for (var i = 0; i < fs.length; i++) {
      ...
      if (v.getClass().getName() === '[B') {
        ...
        if (v.length > 100) {
          ...
          try {
            var f2 = Java.use('java.io.FileOutputStream').$new(DIR + 'v48_plaintext.txt', false);
...
Java.perform(function () {
  ...
  function go() {
    ...
    // ② 不调 init（App 已自己 init 过），只读它当前的 userKey
    W('② App 已初始化的 userKey（getPreUserKey）= [' + clip(inst.getPreUserKey(), 80) + ']');
    try { W('   currentUserKey() = [' + clip(inst.currentUserKey(), 80) + ']'); } catch (e) { W('   currentUserKey 不可用'); }
    try { W('   urk() = [' + clip(inst.urk(), 80) + ']'); } catch (e) { }
    try { W('   av() = [' + clip(inst.av(), 80) + ']'); } catch (e) { }
