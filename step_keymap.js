  ...
  for(var i=0;i<fs.length;i++){fs[i].setAccessible(true);var v=fs[i].get(r);
   ...
   if(v.getClass().getName()==='[B'){var s2=null;try{s2=Java.use('java.lang.String').$new(v,'UTF-8')+'';}catch(e){}
    ...
    if(v.length>100){var cn=0;for(var k=0;k<s2.length;k++){var c=s2.charCodeAt(k);if(c>=0x4e00&&c<=0x9fff)cn++;}
     ...
     try{var f2=Java.use('java.io.FileOutputStream').$new(DIR+'plain_keymap.txt',false);
      f2.write(Java.use('java.lang.String').$new(s2).getBytes('UTF-8'));f2.close();S({m:'★ 明文已落盘 /data/local/tmp/plain_keymap.txt'});}catch(e){}}
...
Java.perform(function(){
 ...
 S({m:'=== 单步 keymap ==='});
 try{S({m:'getKeyMap='+clip(FU.INSTANCE.value.getKeyMap(ctx),300)});}catch(e){S({m:'getKeyMap 异常 '+String(e).slice(0,140)});}
 try{S({m:'loadLocalKey='+clip(FU.INSTANCE.value.loadLocalKey(ctx),300)});}catch(e){}
});
