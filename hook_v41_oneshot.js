...
Java.perform(function () {
  ...
  Java.scheduleOnMainThread(function () {
    ...
    // ★ 用 Android Handler 延时（frida 的 setTimeout 不触发）
    try {
      var Runnable = Java.use('java.lang.Runnable');
      var R = Java.registerClass({
        name: 'com.fock.Later' + Date.now(),
        implements: [Runnable],
        methods: { run: function () { try { phase2(); } catch (e) { W('phase2 异常 ' + e); } } }
      });
      var handler = Java.use('android.os.Handler').$new(Java.use('android.os.Looper').getMainLooper());
      handler.postDelayed(R.$new(), 20000);
      W('③ 已用 Handler.postDelayed 安排 20 秒后执行');
    } catch (e) {
      W('③ Handler 失败: ' + e + ' → 改为立即执行');
      try { phase2(); } catch (e2) { W('立即 phase2 异常 ' + e2); }
    }
  });
